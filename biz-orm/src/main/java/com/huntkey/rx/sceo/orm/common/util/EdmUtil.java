...
public class EdmUtil {
    ...
    /**
     * 设置属性集类的归属主类
     * @param mainClazz 归属的主类
     * @param t
     * @param <T>
     * @return
     */
    public static <T extends PropertyBaseEntity> T setPropertyAffiliatedClass(  Class<? extends BaseEntity> mainClazz,T t)
            throws Exception{
        OrmAccessUtil.accessNull(mainClazz,t);
        TableEnum tableEnum = getEntityTableType(mainClazz);
        if (!TableEnum.Main.equals(tableEnum)){
            throw new RuntimeException("It is not the the main class");
        }
        String clazzName = getEdmClassName(mainClazz);
        t.setClassName(clazzName);
        return t;
    }
