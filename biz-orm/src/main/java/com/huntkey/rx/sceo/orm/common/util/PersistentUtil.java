...
//    }
     ...
     * @throws Exception
     */
    public static <T> List<OrmColumn> getUpdateSqlColumns(T t, Set<String> columnNames, boolean isContainsNullValue) throws Exception {
        //规范更新的columns列
//        columnNames.remove(EdmSysColumn.ID);
//        columnNames.remove(EdmSysColumn.ISDEL);
//        columnNames.remove(EdmSysColumn.CRETIME);
//        columnNames.remove(EdmSysColumn.CREUSER);
//        columnNames.add(EdmSysColumn.MODTIME);
//        columnNames.add(EdmSysColumn.MODUSER);

        List<OrmColumn> allColumns = PersistentUtil.getSQLColumns(t, false, false);

        List<OrmColumn> ormColumns = new ArrayList<>();

        for (OrmColumn column : allColumns) {

            if(!columnNames.contains(column.getColumnName())){
                Field field = FieldUtil.getFieldByColumnName(t.getClass(), column.getColumnName());
                OrmColumn ormColumn = new OrmColumn();
                ormColumn.setColumnName(field.getName());
                ormColumn.setColumnValue(FieldUtil.getFieldValue(t, field));
                ormColumns.add(ormColumn);
            }
        }
        return ormColumns;
    }
